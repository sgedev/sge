...
struct GLEXContext_ {
    ...
    cx_mat4_t viewMatrix;
    GLint viewMatrixLoc;

    cx_mat4_t projectionMatrix;
...
const char *glexVertexShaderSource =
    "#version 330\n"
    "uniform mat4 glexViewMatrix;\n"
    "uniform mat4 glexProjectionMatrix;\n"
    "uniform mat4 glexModelMatrix;\n"
    "layout(location = 0) in vec3 position;\n"
    "layout(location = 1) in vec3 normal;\n"
    "layout(location = 2) in vec2 texCoord;\n"
    "void main(void) {\n"
    "    gl_Position = glexProjectionMatrix * glexViewMatrix * glexModelMatrix * vec4(position, 1.0);\n"
    "}\n";
...
static int glexInitProgram(GLEXContext *context)
{
    ...
    context->modelMatrixLoc = glGetUniformLocation(context->program, "glexModelMatrix");
    context->viewMatrixLoc = glGetUniformLocation(context->program, "glexViewMatrix");
    context->projectionMatrixLoc = glGetUniformLocation(context->program, "glexProjectionMatrix");
...
GLEX_API GLEXContext *glexCreateContext(void)
{
    ...
    context->viewMatrix = cx_mat4d(1.0f);
    context->projectionMatrix = cx_mat4d(1.0f);
...
GLEX_API void glexEndFrame(void)
{
    ...
    if (glex->viewport[2] < 1 || glex->viewport[3] < 1)
        return;
