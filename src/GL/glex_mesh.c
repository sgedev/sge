...
static cx_list_t glexMeshList;
static GLuint glexStaticVertexBuffer;
static GLuint glexStaticIndexBuffer;
static GLuint glexDynamicVertexBuffer;
static GLuint glexDynamicIndexBuffer;
...
GLboolean glexInitMesh(void)
{
	glGenBuffers(1, &glexStaticVertexBuffer);
	glGenBuffers(1, &glexStaticIndexBuffer);
	glGenBuffers(1, &glexDynamicVertexBuffer);
	glGenBuffers(1, &glexDynamicIndexBuffer);

	return GL_TRUE;
}

void glexShutdownMesh(void)
{
	if (glexStaticVertexBuffer) {
		glDeleteBuffers(1, &glexStaticVertexBuffer);
		glexStaticVertexBuffer = 0;
	}

	if (glexStaticIndexBuffer) {
		glDeleteBuffers(1, &glexStaticIndexBuffer);
		glexStaticIndexBuffer = 0;
	}

	if (glexDynamicVertexBuffer) {
		glDeleteBuffers(1, &glexDynamicVertexBuffer);
		glexDynamicVertexBuffer = 0;
	}

	if (glexDynamicIndexBuffer) {
		glDeleteBuffers(1, &glexDynamicIndexBuffer);
		glexDynamicIndexBuffer = 0;
	}
}
