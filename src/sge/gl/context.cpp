...
bool context::init(window *w)
{
	...
	m_context = SDL_GL_CreateContext(w->to_sdl_window());
	if (m_context == NULL) {
		SGE_LOGE("Failed to create OpenGL context.\n");
		return false;
	}

	SDL_GL_MakeCurrent(w->to_sdl_window(), m_context);
...
void context::shutdown(void)
{
	...
	if (m_context == SDL_GL_GetCurrentContext())
		SDL_GL_MakeCurrent(m_window->to_sdl_window(), NULL);
